...
function loadConfigFile(path) {
  const configPath = path || process.env.UNILLM_CONFIG || resolve(process.cwd(), "unillm.config.json");
...
function startDashboard(options = {}) {
  ...
  const server = createServer(serveDashboard({ path }));
...
function cmdInit() {
  const configPath = resolve2(process.cwd(), "unillm.config.json");
  const envPath = resolve2(process.cwd(), ".env.example");
  let created = 0;
