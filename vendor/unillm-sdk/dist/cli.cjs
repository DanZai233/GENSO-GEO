...
var VERSION = "0.1.0";
var DEFAULT_CONFIG = `{
  // UniLLM \u7EDF\u4E00\u914D\u7F6E \u2014\u2014 \u6240\u6709\u63A5\u5165\u672C\u5305\u7684\u9879\u76EE\u5171\u7528\u8FD9\u4E00\u5957
  // \u5B8C\u6574\u8BF4\u660E\u89C1 README.md;API key \u5EFA\u8BAE\u7528 \${ENV_VAR} \u5F15\u7528\u73AF\u5883\u53D8\u91CF,\u4E0D\u8981\u660E\u6587\u5199\u5728\u8FD9\u91CC
  "provider": "deepseek",
...
`;
var ENV_EXAMPLE = `# unillm \u73AF\u5883\u53D8\u91CF\u793A\u4F8B \u2014\u2014 \u590D\u5236\u4E3A .env \u6216\u5199\u5165 shell profile
# \u7528\u54EA\u4E2A\u5382\u5546\u5C31\u914D\u54EA\u4E2A,\u540C\u65F6\u914D\u591A\u4E2A\u65F6\u6309\u4F18\u5148\u7EA7\u81EA\u52A8\u9009\u62E9:
# openai > anthropic > gemini > deepseek > volcengine > moonshot > qwen > zhipu > xai > groq > mistral > siliconflow

UNILLM_PROVIDER=deepseek
UNILLM_API_KEY=sk-xxxxxxxx
...
# GEMINI_API_KEY=AIza...
# DEEPSEEK_API_KEY=sk-...
# VOLCENGINE_API_KEY=xxxx      # \u706B\u5C71\u65B9\u821F ARK_API_KEY \u4EA6\u53EF
# MOONSHOT_API_KEY=sk-...
# DASHSCOPE_API_KEY=sk-...     # \u901A\u4E49\u5343\u95EE
# ZHIPU_API_KEY=xxxx           # \u667A\u8C31 GLM
...
function printHelp() {
  console.log(`
UniLLM SDK v${VERSION} \u2014\u2014 \u7EDF\u4E00\u5927\u6A21\u578B\u63A5\u5165\u5DE5\u5177

\u7528\u6CD5:
  unillm init                 \u5728\u5F53\u524D\u76EE\u5F55\u751F\u6210 unillm.config.json \u4E0E .env.example
  unillm dashboard [--port N] \u542F\u52A8\u53EF\u89C6\u5316 Dashboard(\u9ED8\u8BA4\u7AEF\u53E3 3788,\u81EA\u52A8\u6253\u5F00\u6D4F\u89C8\u5668)
  unillm test [\u5382\u5546]          \u7528\u5F53\u524D\u914D\u7F6E\u5FEB\u901F\u6D4B\u8BD5\u4E00\u6B21\u5BF9\u8BDD
  unillm providers            \u5217\u51FA\u6240\u6709\u652F\u6301\u7684\u5382\u5546
  unillm models <\u5382\u5546>        \u67E5\u770B\u67D0\u5382\u5546\u7684\u9ED8\u8BA4\u6A21\u578B\u5217\u8868
  unillm help                 \u663E\u793A\u5E2E\u52A9

test \u9009\u9879:
  --model <\u540D\u79F0>   \u6307\u5B9A\u6A21\u578B
  --prompt <\u6587\u672C>  \u81EA\u5B9A\u4E49\u63D0\u793A\u8BCD(\u9ED8\u8BA4: \u8BF7\u7528\u4E00\u53E5\u8BDD\u4ECB\u7ECD\u4F60\u81EA\u5DF1)
  --json           \u4EE5 JSON \u6A21\u5F0F\u8BF7\u6C42
  --stream         \u4EE5\u6D41\u5F0F\u8F93\u51FA

\u793A\u4F8B:
  DEEPSEEK_API_KEY=sk-xxx unillm test deepseek --prompt "\u4F60\u597D"
  unillm test --model gpt-4o-mini
`);
}
function cmdInit() {
  const configPath = (0, import_node_path2.resolve)(process.cwd(), "unillm.config.json");
  const envPath = (0, import_node_path2.resolve)(process.cwd(), ".env.example");
  let created = 0;
  if ((0, import_node_fs2.existsSync)(configPath)) {
    console.log(`\u5DF2\u5B58\u5728: ${configPath}(\u8DF3\u8FC7)`);
  } else {
    (0, import_node_fs2.writeFileSync)(configPath, DEFAULT_CONFIG);
    console.log(`\u5DF2\u521B\u5EFA: ${configPath}`);
    created++;
  }
  if ((0, import_node_fs2.existsSync)(envPath)) {
    console.log(`\u5DF2\u5B58\u5728: ${envPath}(\u8DF3\u8FC7)`);
  } else {
    (0, import_node_fs2.writeFileSync)(envPath, ENV_EXAMPLE);
    console.log(`\u5DF2\u521B\u5EFA: ${envPath}`);
    created++;
  }
  console.log(`
\u4E0B\u4E00\u6B65:
  1. \u6253\u5F00 Dashboard \u53EF\u89C6\u5316\u914D\u7F6E\u5E76\u6D4B\u8BD5:   unillm dashboard
  2. \u6216\u76F4\u63A5\u6539 unillm.config.json + \u8BBE\u7F6E\u73AF\u5883\u53D8\u91CF
  3. \u5728\u4EE3\u7801\u91CC\u63A5\u5165:
...
function cmdProviders() {
  ...
  const rows = PROVIDERS.map((p) => {
    const key = p.needsApiKey ? p.envKey[0] || "\u9700\u624B\u52A8\u4F20" : "\u65E0\u9700 Key";
    return `  ${p.id.padEnd(12)} ${p.label.padEnd(34)} key: ${key.padEnd(18)} ${p.defaultBaseUrl}`;
  });
  console.log(rows.join("\n"));
  console.log("\n\u4F7F\u7528 unillm models <\u5382\u5546> \u67E5\u770B\u9ED8\u8BA4\u6A21\u578B\u5217\u8868\u3002\n");
}
function cmdModels(providerId) {
  if (!providerExists(providerId)) {
    console.error(`\u672A\u77E5\u5382\u5546: ${providerId}
\u53EF\u7528: ${PROVIDERS.map((p) => p.id).join(", ")}`);
    process.exit(1);
  }
...
${meta.label} \u9ED8\u8BA4\u6A21\u578B:
`);
  for (const m of meta.defaultModels) console.log(`  - ${m}`);
  console.log(`
Base URL: ${meta.defaultBaseUrl}
`);
}
function parseTestArgs(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    const arg = args[i];
    if (arg === "--model" && args[i + 1]) opts.model = args[++i];
    else if (arg === "--prompt" && args[i + 1]) opts.prompt = args[++i];
    else if (arg === "--json") opts.json = true;
    else if (arg === "--stream") opts.stream = true;
    else if (!arg.startsWith("--")) opts.provider = arg;
  }
  return opts;
}
async function cmdTest(args) {
  const opts = parseTestArgs(args);
  if (opts.provider && !providerExists(opts.provider)) {
    console.error(`\u672A\u77E5\u5382\u5546: ${opts.provider}
\u53EF\u7528: ${PROVIDERS.map((p) => p.id).join(", ")}`);
    process.exit(1);
  }
  ...
  if (meta.needsApiKey && !config.apiKey) {
    console.error(`
\u672A\u627E\u5230 ${config.provider} \u7684 API key\u3002
\u8BF7\u8BBE\u7F6E\u73AF\u5883\u53D8\u91CF ${meta.envKey.join(" / ")} \u6216 UNILLM_API_KEY,\u6216\u8FD0\u884C unillm dashboard \u53EF\u89C6\u5316\u914D\u7F6E\u3002
    `);
    process.exit(1);
  }
  const llm = new UnifiedLLM(opts.provider ? { provider: opts.provider, model: opts.model } : { model: opts.model });
  const prompt = opts.prompt || "\u8BF7\u7528\u4E00\u53E5\u8BDD\u4ECB\u7ECD\u4F60\u81EA\u5DF1,\u5E76\u8BF4\u660E\u4F60\u80FD\u5E2E\u6211\u505A\u4EC0\u4E48\u3002";
  console.log(`
  \u25B8 \u5382\u5546: ${meta.label}`);
  console.log(`  \u25B8 \u6A21\u578B: ${config.model || meta.defaultModels[0] || "(\u9ED8\u8BA4)"}`);
  console.log(`  \u25B8 \u6765\u6E90: ${config.providerSource}`);
  console.log(`  \u25B8 \u63D0\u793A: ${prompt.slice(0, 60)}${prompt.length > 60 ? "\u2026" : ""}
`);
  console.log("  \u56DE\u590D:");
  const startedAt = Date.now();
  if (opts.stream) {
    let full = "";
    for await (const delta of llm.chatStream(prompt)) {
      full += delta;
      process.stdout.write(delta);
    }
    console.log(`

  \u2713 \u5B8C\u6210,\u8017\u65F6 ${((Date.now() - startedAt) / 1e3).toFixed(1)}s,\u5171 ${full.length} \u5B57\u7B26
`);
  } else if (opts.json) {
    const result = await llm.generateJson(prompt);
    console.log(JSON.stringify(result, null, 2));
    console.log(`
  \u2713 \u5B8C\u6210,\u8017\u65F6 ${((Date.now() - startedAt) / 1e3).toFixed(1)}s
`);
  } else {
...
async function main() {
  ...
  switch (cmd) {
    ...
    case "models":
      if (!rest[0]) {
        console.error("\u7528\u6CD5: unillm models <\u5382\u5546>");
        process.exit(1);
      }
      cmdModels(rest[0]);
      break;
